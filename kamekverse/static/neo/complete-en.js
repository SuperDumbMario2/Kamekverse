...
})(window, document, 'script', '//web.archive.org/web/20160819210724/https://www.google-analytics.com/analytics.js', 'ga');
...
(function(a, b) {
	b.init || (b.init = a.Deferred(function() {
	...
	}, b.Entry.toggleNah = function(a) {
		var c = b.Entry.isNahAdded(a),
			d = !c,
			e = a.attr("data-action");
		c && (e += ".delete"), a.trigger("olv:entry:nah:toggle", [d]);
		var f = b.Form.post(e, null, a).done(function() {
			a.trigger("olv:entry:nah:toggle:done", [d])
		}).fail(function() {
			a.trigger("olv:entry:nah:toggle:fail", [c])
		});
		return f
	}, b.Entry.isNahAdded = function(a) {
		return a.hasClass("nah-added")
	}, b.Entry.onNahClick = function(c) {
		if (!c.isDefaultPrevented()) {
			c.preventDefault();
			var d = a(this);
			b.Form.isDisabled(d) || b.Entry.toggleNah(d)
		}
	}, b.Entry.onNahToggle = function(c, d) {
		var e = a(this);
		e.toggleClass("nah-added", d);
		var f = e.attr("data-feeling") || "normal";
		e.find(".nah-button-text").text(b.loc("olv.portal.notmii." + f + (d ? ".delete" : "")));
		var g;
		g = +e.attr("data-is-in-reply-list") ? e.closest(".reply-meta").find(".nah-count") : e.closest(".post-meta").find(".nah-count"), g.text(+g.text() + (d ? 1 : -1));
		var h = a(document).find("#js-my-nah-count");
		if (h[0] && h.text(+h.text() + (d ? 1 : -1)), b.Utils.isIE8AndEarlierStyle) {
			var i = e.closest(".post-meta").find(".nah");
			i.addClass("changing"), setTimeout(function() {
				i.removeClass("changing")
			}, 0)
		}
	}, b.Entry.setupNahButtons = function(c) {
		a(document).on("olv:entry:nah:toggle olv:entry:nah:toggle:fail", ".nah-button", b.Entry.onNahToggle), a(document).on("click", ".nah-button", b.Entry.onNahClick), c.done(function() {
			a(document).off("olv:entry:nah:toggle olv:entry:nah:toggle:fail", ".nah-button", b.Entry.onNahToggle), a(document).off("click", ".nah-button", b.Entry.onNahClick)
		})
	}, b.Entry.setupPostNahButton = function(c) {
		function d(c, d) {
			b.Entry.onNahToggle.apply(this, arguments);
			var e = a(c.target);
			if (!+e.attr("data-is-in-reply-list")) {
				var f = a("#nah-content"),
					g = +e.closest(".post-meta").find(".nah-count").text();
				f.find(".visitor").toggle(d), f.find(".extra").toggle(!d), f.toggleClass("none", 0 === g)
			}
		}
		a(document).on("olv:entry:nah:toggle olv:entry:nah:toggle:fail", ".nah-button", d), a(document).on("click", ".nah-button", b.Entry.onNahClick), c.done(function() {
			a(document).off("click", ".nah-button", b.Entry.onNahClick), a(document).off("olv:entry:nah:toggle olv:entry:nah:toggle:fail", ".nah-button", d)
		})
		...
		var e = a("#post-form");
		e.hasClass("for-identified-user") || (b.find(".js-cannnot-topic-post").removeClass("none"), b.find(".js-feeling-selector").addClass("none"),
			b.find(".js-topic-categories-container").addClass("none"), b.find(".js-post-form-spoiler").addClass("none"), b.find('input[type="text"],textarea').prop("readonly", !0)), b.toggleClass("folded")
		...
		b.Content.autopagerize(".js-post-list", e), b.Entry.setupEmpathyButtons(e), b.Entry.setupNahButtons(e), b.Entry.setupHiddenContents(e), a("form.search").on("submit", b.Form.validateValueLength);
	...
	}), b.router.connect("^/titles/[0-9]+/[0-9]+(/diary|/new|/hot|/in_game|/old|/cold)?$", function(c, d, e) {
		function f() {
	...
	}), b.router.connect("^/titles/[0-9]+/[0-9]+(/artwork(/hot|/new|/cold)?|/topic(/new|/open)?)$", function(c, d, e) {
		function f(d, f) {
		...
		var g = a("#reply-form");
		b.Guest.isGuest() || (b.Entry.setupPostEmpathyButton(e), b.Entry.setupPostNahButton(e), b.Entry.setupEditButtons(e), b.EntryForm.setupSubmission(g, e), b.EntryForm.setupFormStatus(g, e)), b.Entry.setupBodyLanguageSelector(e), a(document).on("olv:report:done", f), e.done(function() {
		...
		var i = a("#post-form");
		b.Guest.isGuest() || (b.Entry.setupEmpathyButtons(e), b.Entry.setupNahButtons(e), b.EntryForm.setupSubmission(i, e), b.EntryForm.setupFormStatus(i, e), i.hasClass("for-identified-user") && b.EntryForm.setupIdentifiedUserForm(i, e)), a(document).on("olv:report:done", h), i.on("olv:entryform:post:done", f);
		...
		a(document).on("click", "[data-track-action]", function(c) {
			...
			var yeah = a(b.target);
			var nah = yeah.closest('.post-meta').find('.nah-button');
			if (nah.length) {
				nah.prop("disabled", c)
			}
		}), a(document).on("olv:entry:nah:toggle", function(b, c) {
			var nah = a(b.target);
			var yeah = nah.closest('.post-meta').find('.empathy-button');
			if (yeah.length) {
				yeah.prop("disabled", c)
			}
		}), a(document).on("olv:entry:empathy:toggle", function(b, c) {
			var yeah = a(b.target);
			var isReply = yeah.attr("data-is-in-reply-list") === "1";
			var nah = yeah.closest(isReply ? '.reply-meta' : '.post-meta').find('.nah-button');
			if (nah.length) {
				nah.prop("disabled", c)
			}
		}), a(document).on("olv:entry:nah:toggle", function(b, c) {
			var nah = a(b.target);
			var isReply = nah.attr("data-is-in-reply-list") === "1";
			var yeah = nah.closest(isReply ? '.reply-meta' : '.post-meta').find('.empathy-button');
			if (yeah.length) {
				yeah.prop("disabled", c)
			}
		})
...
Olv.Locale.Data = {
	...
	"olv.portal.miitoo.surprised.delete": {
		"value": "Unyeah"
	},"olv.portal.notmii.frustrated": {
		"value": "Nah!"
	},
	"olv.portal.notmii.frustrated.delete": {
		"value": "Un-nah"
	},
	"olv.portal.notmii.happy": {
		"value": "Nah..."
	},
	"olv.portal.notmii.happy.delete": {
		"value": "Un-nah"
	},
	"olv.portal.notmii.like": {
		"value": "Nah?!"
	},
	"olv.portal.notmii.like.delete": {
		"value": "Un-nah"
	},
	"olv.portal.notmii.normal": {
		"value": "Nah..."
	},
	"olv.portal.notmii.normal.delete": {
		"value": "Un-nah"
	},
	"olv.portal.notmii.puzzled": {
		"value": "Nah!"
	},
	"olv.portal.notmii.puzzled.delete": {
		"value": "Un-nah"
	},
	"olv.portal.notmii.surprised": {
		"value": "Nah!?"
	},
	"olv.portal.notmii.surprised.delete": {
		"value": "Un-nah"
	},
	"olv.portal.ok": {
		"value": "OK"
